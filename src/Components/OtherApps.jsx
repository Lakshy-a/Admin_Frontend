...
function OtherApps( {data, darkMode}) {  // Destructuring props to directly get `data`
    ...
    return (
        <>
            <div className="container w-full h-fit grid grid-rows-3 grid-cols-3 gap-4 mt-4 items-center cursor-pointer ">
                {data.map((app, index) => {
                    return (
                        <div key={index} className={`${!darkMode ? 'bg-white' : 'bg-gray-900 border-black'} hover:bg-blue-100 rounded-lg border p-3 flex flex-col justify-center items-center gap-2`}>
